...
int p_assignment(ExecutionData E, DecryptedGradebook* gradebook, int index){
  ...
  printf("\n Assignment Name: %s\n", gradebook -> assignments[index].assignmentname);
  printf("\n Assignment Points: %d\n", gradebook -> assignments[index].maxpoints);
  printf("\n Weight: %g\n", gradebook -> assignments[index].weight);
  if (E.grade_order == true) {
    printf("\n Sorted by Grades.\n");
  } else if (E.alpha_order == true) {
    printf("\n Sorted by Last Name then First Name.\n");
  }
  printf("\n %-*s | %-*s | %s\n",
    max_lastname_len, "Last Name",
    max_firstname_len, "First Name",
    "Grade");
  printf("%.*s", max_firstname_len + max_lastname_len + 25,\
    "---------------------------------------------------------------------------\
    ----------------------------------------------------------------------------");
  printf("\n");
  for (i = 0; i < tupl_index; i++) {
    printf(" %-*s | %-*s | %d\n",
      max_lastname_len, tupl[i].lastname,
      max_firstname_len, tupl[i].firstname,
      tupl[i].grade);
  }
  printf("%.*s", max_firstname_len + max_lastname_len + 25,\
    "---------------------------------------------------------------------------\
...
int p_student(ExecutionData E, DecryptedGradebook* gradebook, int index){
  ...
  printf("\n Student Name: %s %s\n",
    gradebook -> students[index].firstname,
    gradebook -> students[index].lastname);
  printf("\n %-*s | %s\n",
    max_assignmentname_len, "Assignment Name",
    "Grade");
  printf("%.*s", max_assignmentname_len + 22,\
    "---------------------------------------------------------------------------\
    ----------------------------------------------------------------------------");
  printf("\n");
  for (i = 0; i < tupl_index; i++) {
    printf(" %-*s | %d (%d/%d)\n",
      max_assignmentname_len, tupl[i].assignmentname,
      tupl[i].grade, tupl[i].grade, tupl[i].max_grade);
  }
  printf("%.*s", max_assignmentname_len + 22,\
    "---------------------------------------------------------------------------\
    ----------------------------------------------------------------------------");
...
int p_final(ExecutionData E, DecryptedGradebook* gradebook){
  ...
  printf("\n Total Weight of All Assignments: %3.2f\n", total_weight);
  if (E.grade_order == true) {
    printf("\n Sorted by Grades.\n");
  } else if (E.alpha_order == true) {
    printf("\n Sorted by Last Name then First Name.\n");
  ...
  printf("\n %-*s | %-*s | %s\n",
    ...
    max_firstname_len, "First Name",
    "Grade");
  printf("%.*s", max_firstname_len + max_lastname_len + 27,\
    "---------------------------------------------------------------------------\
    ----------------------------------------------------------------------------");
  printf("\n");
  for (i = 0; i < tupl_index; i++) {
